...
def extract_features_hog(image, cspace='RGB', orient=9, pix_per_cell=8, cell_per_block=2, hog_channel=0):
    ...
    if hog_channel == 'ALL':
        ...
        for channel in range(feature_image.shape[2]):
            hog_features_, img_ = get_hog_features(feature_image[:,:,channel], orient, pix_per_cell, cell_per_block, vis=True, feature_vec=True)
            hog_features.append(hog_features_)
            img_output.append(img_)
        hog_features = np.ravel(hog_features)        
    else:
        hog_features, img = get_hog_features(feature_image[:,:,hog_channel], orient, pix_per_cell, cell_per_block, vis=True, feature_vec=True)
...
def extract_images_features(imgs, **kwargs):
    ...
    for file in imgs:
        ...
        file_features = extract_image_features(image, **kwargs)
...
def extract_image_features(img, cspace, spatial_size, hist_nbins, orient,  pix_per_cell, cell_per_block, hog_channel, use_hog=True, use_spatial=True, use_hist=True):   
    ...
    if use_hist == True:
        hist_features = color_hist(feature_image, nbins=hist_nbins)
