    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          const double LENGTH_PER_ITERATION = 0.02; // 20ms

          // iterate through each other car
          for ( auto i = 0; i < sensor_fusion.size(); i++ ) {
              ...
              if (car_lane < 0) {
                continue; // don't care this car
              }
              // Find car speed.
              double vx = sensor_fusion[i][3];
              double vy = sensor_fusion[i][4];
              double s_ = sqrt(vx*vx + vy*vy);
              double other_car_s = sensor_fusion[i][5];

              // Estimate car s position after executing previous trajectory.
              other_car_s += prev_size * LENGTH_PER_ITERATION * s_;
              ...
              if ( car_lane == lane ) {
                ...
                // Car right
                car_righ |= car_s - 30 < other_car_s && car_s + 30 > other_car_s;
              }
          }
          ...
          if ( car_ahead ) { // Car ahead
            if ( !car_left && lane != 0 ) {
              ...
              speed_diff -= MAX_ACC; // otherwise we have to slow down
            }
          } else {
